(function ($, global) {
	...
	var updateHeaders = function () {
		...
		var success = (navigator.connection.type != Connection.NONE) ? SUCCESS : DISCONNECTED;
		if (currentTime - lastUpdateCheck > 300000 || online !== (success == SUCCESS)) {
			lastUpdateCheck = currentTime;
			online = success == SUCCESS;

			if (global.device.platform == 'iOS' && parseInt((global.device.version + '').substr(0, (global.device.version + '').indexOf('.')), 10) >= 7) {
				if (global.StatusBar && global.StatusBar.overlaysWebView && global.StatusBar.styleDefault) {
					if (global.mobileNativeConfig && global.mobileNativeConfig.statusTextIsDark) {
						global.StatusBar.styleDefault();
					} else {
						global.StatusBar.styleLightContent();
					}
					global.StatusBar.overlaysWebView(true);
					$('body').addClass('ios native ios-native-behind-statusbar');
				}
			}
